...
for post_id in df_semente['Post_ID']:
    ...
    try:
        ...
        # Mostra erro HTTP antes de tentar interpretar JSON
        if response.status_code != 200:
            print(
                f"[META ERRO] Post {post_id} | "
        ...
        # Meta pode retornar erro dentro do JSON
        ...
        for insight in insights:
            ...
            if nome_metrica not in metrics:
                continue

            values = insight.get('values', [])

            if values:
                metrics[nome_metrica] = values[0].get(
                    'value',
                    0
                )
        ...
        print(
            f"[META EXCEPTION] Post {post_id}: {e}"
        )
    ...
    # Opcional: padronização de typo
    # "carrosel" e "carrossel"
    df_cliques['sub_id2'] = (
        df_cliques['sub_id2']
